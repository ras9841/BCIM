...
function Experiment(path::AbstractString, ntrials::Int64, dc::DimensionlessConst, timestamp::Bool)
...
function Experiment(path::AbstractString, ntrials::Int64, dc::DimensionlessConst, s::System, timestamp::Bool)
  if timestamp
    path = "$path-$(Libc.strftime("%m-%d-%y-%H%M", time()))"
  end
